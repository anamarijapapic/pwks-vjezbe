...
const names = countries.map(element => element.name.common);
console.log("Country names:", names)


// 3.

const europeanCountries = countries.filter(element => element.region === "Europe");
console.log("Countries in Europe:", europeanCountries);


// 4.

const calculateArea = countries => countries.reduce((result, { area: a }) => result += a, 0);
console.log("Total area of european countries (in m^2):", calculateArea(europeanCountries))


// 5.

let findTargetedCountry = capitalCity => {
    const targetedCountry = countries.find(element => element.capital.includes(capitalCity));
    return targetedCountry || null;
}

console.log("Targeted country:", findTargetedCountry("Pyongyang"));